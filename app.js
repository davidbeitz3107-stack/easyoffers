...
let cloud={client:null,user:null,ready:false,syncTimer:0};
...
let st={page:'home',step:1,o:null,photos:[],settingsTab:'company',catalogSearch:'',catalogCategory:'alle',calendarMonth:new Date().getMonth(),calendarYear:new Date().getFullYear()};
function save(){localStorage.setItem(K,JSON.stringify(db));if(cloud.ready&&cloud.client&&cloud.user){clearTimeout(cloud.syncTimer);cloud.syncTimer=setTimeout(syncCloud,450)}}
...
async function loadCloud(user){cloud.user=user;const {data:remote,error}=await cloud.client.from('easyoffer_data').select('data').eq('user_id',user.id).maybeSingle();if(error){authScreen('login','Die Cloud-Daten konnten nicht geladen werden: '+error.message);return}if(remote?.data){db=normalize(remote.data);db.appointments=Array.isArray(db.appointments)?db.appointments:[]}cloud.ready=true;if(!remote)await syncCloud();render()}
async function bootApp(){if(!window.supabase){render();return}cloud.client=window.supabase.createClient(SUPABASE_URL,SUPABASE_PUBLIC_KEY);const {data:{session}}=await cloud.client.auth.getSession();if(session)await loadCloud(session.user);else authScreen()}
async function signOut(){if(cloud.client)await cloud.client.auth.signOut();cloud.user=null;cloud.ready=false;authScreen()}
function nav(){return `<aside class="side"><div class="brand"><div class="logo">E</div><div><b>easyoffer</b><small>Angebote & Kalkulation</small></div></div><nav>${[['home','⌂','Übersicht'],['new','＋','Neues Angebot'],['offers','▤','Angebote'],['calendar','◷','Kalender'],['customers','♙','Kunden'],['catalog','◫','Preiskatalog'],['stats','◈','Statistik'],['settings','⚙','Einstellungen']].map(([p,i,t])=>`<button class="nav ${st.page===p?'active':''}" onclick="${p==='new'?'newOffer()':`go('${p}')`}">${i} <span>${t}</span></button>`).join('')}</nav><div class="sideBottom"><div class="trial"><b>${cloud.user?'Cloud aktiv':'EasyOffer'}</b><span>${cloud.user?esc(cloud.user.email):'Vom Kundenkontakt zum Angebot.'}</span>${cloud.user?'<button class="signOut" onclick="signOut()">Abmelden</button>':''}</div></div></aside>`}
...
Object.assign(window,{
  ...
  printOffer,markOpen,persist,authScreen,submitAuth,signOut
});
bootApp();
