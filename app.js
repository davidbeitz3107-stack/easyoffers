...
function offerDeadline(o){const until=offerValidUntil(o),today=new Date();today.setHours(0,0,0,0);const days=Math.ceil((until-today)/86400000),active=!['angenommen','abgelehnt'].includes(o.status);const follow=o.followUp?` · Nachfassen: ${dateText(o.followUp)}`:'';if(active&&days<0)return `<small class="deadline overdue">Abgelaufen am ${dateText(until)}${follow}</small>`;if(active&&days<=7)return `<small class="deadline soon">Gültig bis ${dateText(until)}${follow}</small>`;return `<small>Gültig bis ${dateText(until)}${follow}</small>`}
