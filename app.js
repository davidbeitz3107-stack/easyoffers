...
function offerTemplates(){return [...SYSTEM_TEMPLATES,...db.templates]}
...
function saveOfferAsTemplate(){const name=prompt('Name für diese Vorlage, z. B. Badmodernisierung');if(!name?.trim())return;db.templates.push({id:uid(),name:name.trim(),title:st.o.title||'Angebot',items:JSON.parse(JSON.stringify(st.o.items||[])),created:new Date().toISOString()});save();toast('Vorlage gespeichert')}
function removeOfferTemplate(id){if(!confirm('Diese eigene Vorlage wirklich löschen?'))return;db.templates=db.templates.filter(template=>template.id!==id);save();render()}
function customer(o){
