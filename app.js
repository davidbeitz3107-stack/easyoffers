...
function row(o){return `<div class="row" onclick="openOffer('${o.id}')"><b>#${o.no}</b><div><b>${esc(o.customer.name||'Unbenannter Kunde')}</b><br><span>${esc(o.title)}</span>${o.reference?`<br><small>${esc(projectText(o))}</small>`:''}<br>${offerDeadline(o)}</div><span class="badge ${o.status==='angenommen'?'open':''}">${statusLabel(o.status)}</span><strong>${eur(net(o))}</strong><b>›</b></div>`}
function newOffer(customerId=''){const master=db.customers.find(c=>c.id===customerId);st.o={id:uid(),no:db.settings.next++,reference:'',customerId:master?.id||'',customer:master?{name:master.name,email:master.email,phone:master.phone,address:master.address}:{name:'',email:'',phone:'',address:''},request:'',title:db.settings.offerTitle||'Angebot',items:[],notes:'',attachments:[],status:'entwurf',vat:db.settings.vat||19,discountType:'percent',discount:0,validity:db.settings.offerValidity||14,followUp:''};st.page='new';st.step=1;save();render()}
...
function printOffer(){
  const o=st.o,s=db.settings,check=offerCheckState(o);if(check.required.length){toast('Bitte zuerst die Pflichtpunkte im Angebots-Check ergänzen.');return}const n=net(o),g=gross(o),offerNo=`${s.offerPrefix||'ANG-'}${o.no}`;
...
body{font:12px/1.5 Arial,Helvetica,sans-serif;color:#1a2a3b;background:#fff}
...
.header .company{margin-top:7px;font-size:19px;letter-spacing:-.25px}.header .muted{color:#627287;font-size:12px;line-height:1.55}.header .logo{max-width:125px;max-height:50px;margin:0 0 12px;padding:0;background:transparent;border-radius:0}
...
@media print{body{font-size:12px}.page{max-width:none}.header,.addressRow,.summary{break-inside:avoid}.header{padding-top:7px}}
