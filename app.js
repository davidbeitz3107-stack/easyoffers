...
db.templates=Array.isArray(db.templates)?db.templates.filter(template=>template&&template.id&&template.name).map(template=>({...template,items:Array.isArray(template.items)?template.items:[]})):[];
...
async function saveMyOfferContact(){const name=$('#myOfferContact')?.value.trim();if(!name)return toast('Bitte deinen Namen eingeben.');if(!cloud.client||!cloud.user)return toast('Bitte zuerst anmelden.');const {data,error}=await cloud.client.auth.updateUser({data:{...(cloud.user.user_metadata||{}),full_name:name}});if(error)return toast('Name konnte nicht gespeichert werden.');cloud.user=data.user||{...cloud.user,user_metadata:{...(cloud.user.user_metadata||{}),full_name:name}};toast('Dein Ansprechpartnername wurde gespeichert.');render()}
...
function onboardingPage(){const s=db.settings,step=st.onboardingStep;const steps=['Betrieb','Angebote','Preiskatalog'];let content='';if(step===1)content=`<div class="card formgrid"><div class="wide"><span class="eyebrow">SCHRITT 1 VON 3</span><h1>Dein Betrieb.</h1><p class="lead">Diese Angaben erscheinen später auf Angeboten und im PDF.</p></div>${field('Firmenname *','onCompany',s.company==='Dein Betrieb'?'':s.company,'Musterbetrieb GmbH')}${field('Ansprechpartner','onOwner',s.owner,'Max Mustermann')}${field('E-Mail','onEmail',s.email,'info@betrieb.de')}${field('Telefon','onPhone',s.phone,'01234 56789')}${field('Adresse','onAddress',s.address,'Straße, PLZ Ort')}</div>`;if(step===2)content=`<div class="card formgrid"><div class="wide"><span class="eyebrow">SCHRITT 2 VON 3</span><h1>Deine Angebotsregeln.</h1><p class="lead">Du kannst alles später unter Einstellungen ändern.</p></div><label class="field"><span>MwSt. (%)</span><input id="onVat" type="number" value="${s.vat||19}"></label>${field('Angebotsnummer-Präfix','onPrefix',s.offerPrefix||'ANG-','ANG-')}<label class="field"><span>Gültigkeit (Tage)</span><input id="onValidity" type="number" value="${s.offerValidity||14}"></label><label class="field"><span>Zahlungsziel (Tage)</span><input id="onPayment" type="number" value="${s.paymentTerm||14}"></label></div>`;if(step===3)content=`<div class="card onboardingCatalog"><span class="eyebrow">SCHRITT 3 VON 3</span><h1>Preiskatalog starten.</h1><p class="lead">Du kannst deine bestehende CSV- oder Excel-Preisliste später direkt im Preiskatalog importieren.</p><label class="onboardingChoice"><input type="radio" name="catalogStart" value="starter" checked><span><b>Mit Beispielpositionen starten</b><small>Du bekommst einige Musterartikel und kannst sie direkt anpassen.</small></span></label><label class="onboardingChoice"><input type="radio" name="catalogStart" value="empty"><span><b>Ohne Artikel starten</b><small>Du legst eigene Positionen an oder importierst deine Preisliste.</small></span></label></div>`;return `<div class="onboarding"><div class="onboardingProgress">${steps.map((x,i)=>`<span class="${i+1===step?'current':i+1<step?'done':''}"><i>${i+1<step?'✓':i+1}</i>${x}</span>`).join('')}</div>${content}<div class="actions"><button class="ghost" ${step===1?'disabled':''} onclick="onboardingBack()">← Zurück</button><button class="primary" onclick="onboardingNext()">${step===3?'Einrichtung abschließen':'Weiter →'}</button></div></div>`}
function onboardingBack(){if(st.onboardingStep>1){st.onboardingStep--;render()}}
async function renameWorkspace(name){if(!cloud.client||!cloud.workspace||!name||cloud.workspace.name===name)return true;if(cloud.workspace.role!=='owner')return false;const {error}=await cloud.client.from('workspaces').update({name}).eq('id',cloud.workspace.id);if(error){console.warn('Arbeitsbereich konnte nicht umbenannt werden',error.message);return false}cloud.workspace.name=name;return true}
async function onboardingNext(){const s=db.settings;if(st.onboardingStep===1){s.company=$('#onCompany').value.trim();s.owner=$('#onOwner').value.trim();s.email=$('#onEmail').value.trim();s.phone=$('#onPhone').value.trim();s.address=$('#onAddress').value.trim();if(!s.company)return toast('Bitte Firmenname eingeben');st.onboardingStep=2;render();return}if(st.onboardingStep===2){s.vat=Number($('#onVat').value)||19;s.offerPrefix=$('#onPrefix').value.trim()||'ANG-';s.offerValidity=Number($('#onValidity').value)||14;s.paymentTerm=Number($('#onPayment').value)||14;st.onboardingStep=3;render();return}if(document.querySelector('input[name="catalogStart"]:checked')?.value==='empty')db.catalog=[];s.onboardingDone=true;const renamed=await renameWorkspace(s.company);cloud.onboarding=false;save();render();toast(renamed?'Einrichtung abgeschlossen':'Einrichtung abgeschlossen – Betriebsname wird beim nächsten Speichern übernommen')}
function layout(c){$('#app').innerHTML=`<div class="app">${nav()}<main>${c}</main></div>`}
function go(p){st.page=p;render()}
function setupTips(){const s=db.settings,company=!!(s.company&&s.company!=='Dein Betrieb'),catalog=db.catalog.length>0,offer=db.offers.length>0;if(s.setupTipsDismissed)return '';const count=[company,catalog,offer].filter(Boolean).length;const item=(done,title,text,action,target)=>`<li class="${done?'done':''}"><i>${done?'✓':'•'}</i><div><b>${title}</b><small>${text}</small></div>${done?'':`<button class="textButton" onclick="openSetupTip('${target}')">${action} →</button>`}</li>`;const allDone=count===3;return `<section class="setupTips card"><div class="setupTipsHead"><div><span class="eyebrow">${allDone?'EINRICHTUNG ABGESCHLOSSEN':'SCHNELL STARTKLAR'}</span><h2>${allDone?'Alles startklar.':'Deine ersten Schritte'}</h2><p>${allDone?'Dein Betrieb ist eingerichtet. Du kannst jederzeit direkt ein neues Angebot erstellen.':`${count}/3 erledigt – damit dein erster Entwurf sofort professionell aussieht.`}</p></div><button class="tipClose" onclick="dismissSetupTips()" title="Hinweise ausblenden">×</button></div><ol>${item(company,'Betriebsdaten prüfen','Firma, Kontakt und Adresse für dein PDF.','Einstellungen','company')}${item(catalog,'Preiskatalog anlegen','Eigene Preise bleiben immer die Grundlage.','Katalog öffnen','catalog')}${item(offer,'Erstes Angebot erstellen','Der Assistent führt dich Schritt für Schritt durch.','Angebot starten','offer')}</ol></section>`}
function openSetupTip(target){if(target==='company'){st.page='settings';st.settingsTab='company'}else if(target==='catalog')st.page='catalog';else if(target==='offer'){newOffer();return}render()}
function dismissSetupTips(){db.settings.setupTipsDismissed=true;save();render()}
function home(){const won=db.offers.filter(o=>o.status==='angenommen');return `<div class="top"><div><span class="eyebrow">EASYOFFER FÜR HANDWERK & SERVICE</span><h1>Deine Angebote.<br><em>Schneller fertig.</em></h1><p class="lead">Kundenanfrage rein. Leistungen prüfen. Angebot raus.</p></div><button class="primary big" onclick="newOffer()">＋ Neues Angebot</button></div><div class="hero"><div class="heroCard"><div class="spark">✦ DIGITALER ANGEBOTSASSISTENT</div><h2>Von der Anfrage zur sauberen Kalkulation.</h2><p>EasyOffer schlägt Leistungen aus deiner eigenen Preisliste vor. Du prüfst, änderst und versendest.</p><button class="primary" onclick="newOffer()">Angebot starten →</button><div class="flow"><span>01 Kunde</span><span>02 Anfrage</span><span>03 Analyse</span><span>04 Kalkulation</span><span>05 Angebot</span></div></div><div class="stats"><div class="stat"><small>ANGEBOTE</small><strong>${db.offers.length}</strong><span>gesamt</span></div><div class="stat"><small>GEWONNEN</small><strong>${eur(won.reduce((a,o)=>a+net(o),0))}</strong><span>Auftragswert netto</span></div><div class="stat"><small>ARTIKEL</small><strong>${db.catalog.length}</strong><span>im Katalog</span></div></div></div>${setupTips()}<section class="section"><div class="sectionHead"><div><h2>Letzte Angebote</h2><p class="lead">Schnell wieder öffnen.</p></div><button class="ghost" onclick="go('offers')">Alle →</button></div><div class="card rows">${db.offers.length?db.offers.slice(-6).reverse().map(row).join(''):`<div class="empty"><h3>Noch kein Angebot</h3><p>Starte mit einer Kundenanfrage.</p><button class="primary" onclick="newOffer()">Erstes Angebot erstellen</button></div>`}</div></section>`}
...
function offerValidUntil(o){const date=new Date(o.created||Date.now());date.setDate(date.getDate()+Number(o.validity||db.settings.offerValidity||14));return date}
...
function offerDeadline(o){const until=offerValidUntil(o),today=new Date();today.setHours(0,0,0,0);const days=Math.ceil((until-today)/86400000),active=!['angenommen','abgelehnt'].includes(o.status),followDate=o.followUp?new Date(`${o.followUp}T00:00:00`):null,followDue=active&&followDate&&followDate<=today;const follow=o.followUp?` · Nachfassen: ${dateText(o.followUp)}`:'';const sent=o.sentAt?` · Versendet: ${dateText(o.sentAt)}`:'';if(followDue)return `<small class="deadline followDue">Nachfassen fällig: ${dateText(o.followUp)} · Gültig bis ${dateText(until)}${sent}</small>`;if(active&&days<0)return `<small class="deadline overdue">Abgelaufen am ${dateText(until)}${follow}${sent}</small>`;if(active&&days<=7)return `<small class="deadline soon">Gültig bis ${dateText(until)}${follow}${sent}</small>`;return `<small>Gültig bis ${dateText(until)}${follow}${sent}</small>`}
function projectText(o){return o.reference?`Projekt / Baustelle: ${o.reference}`:''}
function row(o){const creator=[o.creatorName,o.creatorEmail].filter(Boolean).join(' · ');return `<div class="row" onclick="openOffer('${o.id}')"><b>#${o.no}</b><div><b>${esc(o.customer.name||'Unbenannter Kunde')}</b><br><span>${esc(o.title)}</span>${o.reference?`<br><small>${esc(projectText(o))}</small>`:''}${creator?`<br><small class="offerCreator">Erstellt von: ${esc(creator)}</small>`:''}<br>${offerDeadline(o)}</div><span class="badge ${o.status==='angenommen'?'open':''}">${statusLabel(o.status)}</span><strong>${eur(net(o))}</strong><b>›</b></div>`}
function currentOfferContact(){const email=String(cloud.user?.email||db.settings.email||'').trim(),name=String(cloud.user?.user_metadata?.full_name||'').trim();return {creatorName:name||email.split('@')[0]||db.settings.owner||'',creatorEmail:email}}
function newOffer(customerId=''){const master=db.customers.find(c=>c.id===customerId),contact=currentOfferContact();st.o={id:uid(),no:db.settings.next++,reference:'',customerId:master?.id||'',customer:master?{name:master.name,email:master.email,phone:master.phone,address:master.address}:{name:'',email:'',phone:'',address:''},request:'',title:db.settings.offerTitle||'Angebot',items:[],notes:'',attachments:[],creatorName:contact.creatorName,creatorEmail:contact.creatorEmail,status:'entwurf',vat:db.settings.vat||19,discountType:'percent',discount:0,validity:db.settings.offerValidity||14,followUp:''};st.page='new';st.step=1;save();render()}
...
function customTemplates(){if(!Array.isArray(db.templates))db.templates=[];return db.templates}
function offerTemplates(){return [...SYSTEM_TEMPLATES,...customTemplates()]}
function applyOfferTemplate(id){const template=offerTemplates().find(item=>item.id===id);if(!template)return;st.o.templateId=id;st.o.title=template.title||'Angebot';if(template.items?.length)st.o.items=JSON.parse(JSON.stringify(template.items));else if(template.terms?.length){const used=new Set();st.o.items=template.terms.map(term=>db.catalog.find(item=>[item.name,item.article,item.category].join(' ').toLowerCase().includes(term))).filter(item=>item&&!used.has(item.id)&&used.add(item.id)).map(item=>({catalogId:item.id,name:item.name,qty:1,unit:item.unit,price:+item.price||0,cost:+item.cost||0}))}else st.o.items=[];render();toast(`${template.name} ausgewählt`)}
function saveOfferAsTemplate(){const name=prompt('Name für diese Vorlage, z. B. Badmodernisierung');if(!name?.trim())return;customTemplates().push({id:uid(),name:name.trim(),title:st.o.title||'Angebot',items:JSON.parse(JSON.stringify(st.o.items||[])),created:new Date().toISOString()});save();toast('Vorlage gespeichert')}
function removeOfferTemplate(id){if(!confirm('Diese eigene Vorlage wirklich löschen?'))return;db.templates=customTemplates().filter(template=>template.id!==id);save();render()}
function customer(o){
  const customers=db.customers.slice().sort((a,b)=>(a.company||a.name).localeCompare(b.company||b.name,'de'));
  const picker=customers.length?`<div class="card" style="margin-bottom:16px"><label class="field"><span>Bestehenden Kunden auswählen</span><select onchange="selectExistingCustomer(this.value)"><option value="">Neuer Kunde</option>${customers.map(c=>`<option value="${esc(c.id)}" ${o.customerId===c.id?'selected':''}>${esc(c.company||c.name)}${c.name&&c.company?` · ${esc(c.name)}`:''}${c.no?` (#${c.no})`:''}</option>`).join('')}</select></label></div>`:'';
  const templates=offerTemplates();const selected=templates.find(template=>template.id===o.templateId);
  const templateMatches=template=>template.items?.length||new Set((template.terms||[]).flatMap(term=>db.catalog.filter(item=>[item.name,item.article,item.category].join(' ').toLowerCase().includes(term)).map(item=>item.id))).size;
  const templatePicker=`<div class="templatePicker card"><div class="sectionHead"><div><b>Vorlage wählen <span style="font-weight:500;color:var(--muted)">(optional)</span></b><small style="display:block;color:var(--muted);margin-top:4px">Vorlagen setzen nur passende Positionen aus deinem eigenen Preiskatalog ein.</small></div></div><div class="templateGrid">${templates.map(template=>`<button class="templateCard ${o.templateId===template.id?'selected':''}" onclick="applyOfferTemplate('${template.id}')"><span class="templateIcon">${esc(template.icon||'☆')}</span><span class="templateCategory">${esc(template.category||'Eigene Vorlage')}</span><b>${esc(template.name)}</b><small>${esc(template.description||`${template.items?.length||0} Positionen gespeichert`)}</small><em>${templateMatches(template)} Katalogtreffer</em></button>`).join('')}</div>${selected?`<div class="templateSelected">✓ <b>${esc(selected.name)}</b> gewählt. Nach dem Kunden geht es direkt zur Kalkulation; Positionen bleiben vollständig bearbeitbar.</div>`:''}</div>`;
  return `${templatePicker}${picker}<div class="card formgrid">${field('Kundenname *','name',o.customer.name,'Familie Müller')}${field('Projekt / Baustelle / Referenz','reference',o.reference||'','z. B. Bad EG · K-2026-184')}${field('E-Mail','email',o.customer.email,'kunde@email.de')}${field('Telefon','phone',o.customer.phone,'0561 ...')}${field('Adresse','address',o.customer.address,'Straße, PLZ Ort')}</div><div class="actions"><span></span><button class="primary" onclick="saveCustomer()">${o.templateId?'Zur Kalkulation →':'Weiter →'}</button></div>`
}
function selectExistingCustomer(id){if(!id){st.o.customerId='';return}const customer=db.customers.find(c=>c.id===id);if(!customer)return;st.o.customerId=customer.id;st.o.customer={name:customer.name,email:customer.email,phone:customer.phone,address:customer.address};render()}
const attachmentIcon=file=>String(file.type||'').startsWith('image/')?'▧':String(file.type||'').includes('pdf')?'PDF':'DOC';
const fileSize=size=>size<1024*1024?`${Math.max(1,Math.round(size/1024))} KB`:`${(size/1024/1024).toFixed(1)} MB`;
function attachmentAnalysis(o,file){const result=(o.imageAnalyses||[]).find(item=>item.attachmentId===file.id);if(!result)return '';return `<div class="imageAnalysisResult"><b>✦ Bild-KI: ${esc(result.summary||'Prüfung abgeschlossen')}</b>${result.unmatched?.length?`<small>Erkannt, aber nicht im Katalog: ${result.unmatched.map(esc).join(', ')}</small>`:''}${result.questions?.length?`<small>Zu prüfen: ${result.questions.map(esc).join(' · ')}</small>`:''}${result.positions?.length?`<button class="textButton" onclick="applyImageAnalysis('${file.id}')">${result.positions.length} Position${result.positions.length===1?'':'en'} zur Kalkulation hinzufügen</button>`:`<small>Keine sichere Katalogposition vorgeschlagen.</small>`}</div>`}
function attachmentsPanel(o){const files=Array.isArray(o.attachments)?o.attachments:[];return `<div class="offerAttachments"><div><b>📎 Dateien & Baustellenfotos</b><small>Privat im Arbeitsbereich gespeichert · nie im Angebots-PDF</small></div><label class="attachmentUpload"><input type="file" accept="image/png,image/jpeg,image/webp,application/pdf,.pdf,.doc,.docx" multiple onchange="uploadAttachments(event)"><span>＋ Datei hinzufügen</span></label><small class="attachmentHint">Fotos, PDF oder Word · max. 10 MB je Datei</small>${files.length?`<div class="attachmentList">${files.map(file=>`<div class="attachmentRow"><i>${attachmentIcon(file)}</i><span><b>${esc(file.name)}</b><small>${fileSize(file.size)}${file.created?` · ${dateText(file.created)}`:''}</small></span><button class="textButton" onclick="openAttachment('${file.id}')">Öffnen</button>${String(file.type||'').startsWith('image/')?`<button class="textButton" onclick="analyzeAttachment('${file.id}')">KI prüfen</button>`:''}<button class="textButton dangerText" onclick="removeAttachment('${file.id}')">Entfernen</button>${attachmentAnalysis(o,file)}</div>`).join('')}</div>`:`<p class="attachmentEmpty">Noch keine Dateien hinterlegt.</p>`}</div>`}
function request(o){return `<div class="card"><textarea id="req" class="request" placeholder="z. B. Kunde benötigt eine Reparatur, Montage oder neue Ausstattung.">${esc(o.request)}</textarea>${attachmentsPanel(o)}</div><div class="actions"><button class="ghost" onclick="back()">← Zurück</button><button class="primary" onclick="analyze()">Anfrage analysieren ✦</button></div>`}
function attachmentAllowed(file){const types=['image/png','image/jpeg','image/webp','application/pdf','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document'];return types.includes(file.type)||/\.(png|jpe?g|webp|pdf|docx?)$/i.test(file.name||'')}
async function uploadAttachments(event){const files=[...(event.target.files||[])];event.target.value='';if(!files.length)return;if(!cloud.client||!cloud.user||!cloud.workspace)return toast('Dateien können erst nach der Cloud-Anmeldung gespeichert werden.');if(files.length>5)return toast('Bitte maximal 5 Dateien auf einmal auswählen.');const invalid=files.find(file=>!attachmentAllowed(file)||file.size>10*1024*1024);if(invalid)return toast(!attachmentAllowed(invalid)?'Erlaubt sind Fotos, PDF und Word-Dateien.':'Eine Datei ist größer als 10 MB.');st.o.attachments=Array.isArray(st.o.attachments)?st.o.attachments:[];persist();let uploaded=0;for(const file of files){const safeName=String(file.name||'datei').replace(/[^a-zA-Z0-9._-]/g,'_').slice(-120);const path=`${cloud.workspace.id}/${st.o.id}/${uid()}-${safeName}`;const {error}=await cloud.client.storage.from('offer-files').upload(path,file,{contentType:file.type||undefined,upsert:false});if(error){console.warn('Datei-Upload fehlgeschlagen',error.message);return toast('Datei-Speicher ist noch nicht eingerichtet oder der Upload ist fehlgeschlagen.')}st.o.attachments.push({id:uid(),name:file.name.slice(0,180),path,type:file.type||'',size:file.size,created:new Date().toISOString()});uploaded++}persist();render();toast(`${uploaded} Datei${uploaded===1?'':'en'} gespeichert`)}
async function openAttachment(id){const file=(st.o.attachments||[]).find(item=>item.id===id);if(!file||!cloud.client)return;const {data,error}=await cloud.client.storage.from('offer-files').createSignedUrl(file.path,60);if(error||!data?.signedUrl)return toast('Datei konnte nicht geöffnet werden.');window.open(data.signedUrl,'_blank','noopener')}
async function removeAttachment(id){const file=(st.o.attachments||[]).find(item=>item.id===id);if(!file||!cloud.client)return;if(!confirm(`„${file.name}“ wirklich entfernen?`))return;const {error}=await cloud.client.storage.from('offer-files').remove([file.path]);if(error)return toast('Datei konnte nicht entfernt werden.');st.o.attachments=st.o.attachments.filter(item=>item.id!==id);persist();render();toast('Datei entfernt')}
async function analyzeAttachment(id){const o=st.o,file=(o.attachments||[]).find(item=>item.id===id);if(!file)return;if(!String(file.type||'').startsWith('image/'))return toast('Bild-KI funktioniert nur mit Fotos.');if(db.settings.aiMode!=='cloud')return toast('Bitte unter Einstellungen → KI zuerst „Echte KI“ auswählen.');if(!cloud.client||!cloud.user)return toast('Bitte zuerst anmelden.');const catalog=db.catalog.map(item=>({id:item.id,article:item.article||'',name:item.name||'',category:item.category||'',unit:item.unit||''}));try{toast('Bild-KI prüft das Foto …');const {data,error}=await cloud.client.functions.invoke('analyze-offer-request',{body:{request:analysisRequest(o)||'Bitte prüfe das Baustellenfoto für dieses Angebot.',catalog,imagePath:file.path,imageType:file.type}});if(error)throw error;if(!data||!Array.isArray(data.positions))throw Error('Ungültige KI-Antwort');const positions=data.positions.map(position=>{const item=db.catalog.find(catalogItem=>catalogItem.id===position.catalogId);return item?{catalogId:item.id,name:item.name,qty:Math.max(1,Number(position.quantity)||1),unit:item.unit,price:+item.price||0,cost:+item.cost||0}:null}).filter(Boolean);o.imageAnalyses=(o.imageAnalyses||[]).filter(item=>item.attachmentId!==id);o.imageAnalyses.push({attachmentId:id,summary:String(data.imageSummary||'Foto geprüft.').slice(0,500),positions,unmatched:Array.isArray(data.unmatched)?data.unmatched.slice(0,8).map(String):[],questions:Array.isArray(data.questions)?data.questions.slice(0,3).map(String):[],created:new Date().toISOString()});persist();render();toast('Bild-KI-Ergebnis bereit')}catch(error){console.warn('Bild-KI fehlgeschlagen',error);toast('Bild-KI konnte das Foto nicht prüfen.')}}
function applyImageAnalysis(id){const result=(st.o.imageAnalyses||[]).find(item=>item.attachmentId===id);if(!result?.positions?.length)return;const existing=new Set(st.o.items.map(item=>`${item.catalogId||''}:${item.name}`));let added=0;result.positions.forEach(item=>{const key=`${item.catalogId||''}:${item.name}`;if(!existing.has(key)){st.o.items.push({...item});existing.add(key);added++}});persist();st.step=4;render();toast(added?`${added} Position${added===1?'':'en'} zur Kalkulation hinzugefügt`:'Diese Positionen sind bereits in der Kalkulation.')}
function analysis(o){const missing=Array.isArray(o.missingCatalogItems)?o.missingCatalogItems:[],questions=Array.isArray(o.aiQuestions)?o.aiQuestions:[],hasItems=o.items.length>0,source=o.aiSource==='cloud'?'Echte KI':'Lokale Analyse';return `<div class="analysisCard"><div class="analysisHead"><div><span class="eyebrow">${source.toUpperCase()}</span><h2>Analyse bereit.</h2><p class="lead">${esc(o.title)}</p></div><div class="analysisCount"><b>${o.items.length}</b><span>Vorschläge</span></div></div><div class="analysisNotice ok"><b>✓ Preise bleiben unter deiner Kontrolle</b><p>Alle Preise und Einkaufskosten werden direkt aus deinem eigenen Preiskatalog übernommen.</p></div>${questions.length?`<div class="analysisNotice question"><b>✦ Für ein genaueres Angebot fehlen noch Angaben</b><ol>${questions.map(q=>`<li>${esc(q)}</li>`).join('')}</ol><label class="field"><span>Zusätzliche Angaben</span><textarea id="aiDetails" placeholder="Antworten oder weitere Informationen hier ergänzen …">${esc(o.aiDetails||'')}</textarea></label><button class="ghost refineBtn" onclick="refineAnalysis()">Mit diesen Angaben erneut analysieren</button><small>Die bestehende Anfrage bleibt erhalten. EasyOffer ergänzt nur deine Angaben für die neue Analyse.</small></div>`:''}${missing.length?`<div class="analysisNotice warning"><b>! Katalogpositionen fehlen</b><p>Für folgende Punkte wurde kein passender Artikel gefunden:</p><div class="missingChips">${missing.map(x=>`<span>${esc(x)}</span>`).join('')}</div><small>Ergänze sie im nächsten Schritt manuell oder lege sie später im Preiskatalog an. Es werden keine Preise erfunden.</small></div>`:''}<div class="analysisList"><div class="analysisListHead"><div><b>Vorgeschlagene Positionen</b><small>Bitte Menge und Leistung vor dem Versand prüfen.</small></div><span class="badge">Aus deinem Katalog</span></div>${hasItems?o.items.map(x=>`<div class="analysisItem"><div class="analysisQty">${x.qty}<small>${esc(x.unit)}</small></div><div><b>${esc(x.name)}</b><small>Artikelpreis: ${eur(x.price)} netto</small></div><span class="suggestionTag">KI-Vorschlag</span></div>`).join(''):`<div class="analysisEmpty"><b>Keine passende Katalogposition erkannt.</b><p>Du kannst im nächsten Schritt Positionen aus deinem Katalog hinzufügen.</p></div>`}</div><div class="analysisCheck"><span>${hasItems?'1':'0'}/1</span><div><b>Prüfung vor dem Angebot</b><small>${missing.length?'Fehlende Katalogartikel und Mengen bitte kontrollieren.':'Mengen, technische Details und Termine bitte kurz kontrollieren.'}</small></div></div><div class="actions"><button class="ghost" onclick="back()">← Anfrage</button><button class="primary" onclick="st.step=4;render()">${hasItems?'Vorschläge prüfen →':'Positionen hinzufügen →'}</button></div></div>`}
...
function analysisRequest(o){return `${o.request||''}${o.aiDetails?`\n\nZusätzliche Angaben des Betriebs:\n${o.aiDetails}`:''}`}
function analyzeLocal(o){
  const text=analysisRequest(o);
  o.title=/wärmepumpe/i.test(text)
...
async function analyzeCloud(o){
  ...
  const {data,error}=await cloud.client.functions.invoke('analyze-offer-request',{body:{request:analysisRequest(o),catalog}});
  ...
  o.aiQuestions=Array.isArray(data.questions)?data.questions.slice(0,3).map(value=>String(value).slice(0,180)):[];
  o.aiSource='cloud'
}
...
async function refineAnalysis(){
  const o=st.o;
  const details=$('#aiDetails')?.value.trim()||'';
  if(!details)return toast('Bitte ergänze mindestens eine Angabe.');
  o.aiDetails=details;
  try{
    toast(db.settings.aiMode==='cloud'?'KI analysiert die Ergänzung …':'Analyse wird aktualisiert …');
    if(db.settings.aiMode==='cloud')await analyzeCloud(o);else analyzeLocal(o);
    render();
  }catch(error){console.warn('KI-Nachanalyse fehlgeschlagen',error);toast('KI nicht erreichbar – deine Angaben bleiben erhalten.');}
}
function items(o){
...
function saveOfferNotes(){const input=$('#offerNotes');if(!input)return;st.o.notes=input.value.trim();persist();toast('Interne Notiz gespeichert')}
function offerCheckState(o){const s=db.settings,required=[],warnings=[],ok=[];if(!String(s.company||'').trim()||s.company==='Dein Betrieb')required.push(['Betriebsdaten','Firmenname fehlt auf dem Angebot.']);else ok.push(['Betriebsdaten','Firma wird im Angebot angezeigt.']);if(!String(o.customer?.name||'').trim())required.push(['Kunde','Kundenname fehlt.']);else if(!String(o.customer?.address||'').trim())required.push(['Kundenadresse','Für ein professionelles Angebot ergänzen.']);else ok.push(['Kundendaten','Name und Adresse sind vorhanden.']);if(!(o.items||[]).length)required.push(['Positionen','Es wurde noch keine Position hinzugefügt.']);else{const unnamed=o.items.filter(item=>!String(item.name||'').trim()).length,invalid=o.items.filter(item=>Number(item.qty)<=0).length,free=o.items.filter(item=>Number(item.price)===0).length;if(unnamed)required.push(['Positionen',`${unnamed} Position${unnamed===1?'':'en'} ohne Bezeichnung.`]);else if(invalid)required.push(['Mengen',`${invalid} Position${invalid===1?' hat':'en haben'} keine gültige Menge.`]);else ok.push(['Positionen',`${o.items.length} Position${o.items.length===1?'':'en'} sind enthalten.`]);if(free)warnings.push(['Preis prüfen',`${free} Position${free===1?' hat':'en haben'} einen Preis von 0,00 €.`])}if(!Number(o.validity||s.offerValidity))warnings.push(['Gültigkeit prüfen','Bitte eine Angebotsgültigkeit festlegen.']);else ok.push(['Angebotsregeln',`Gültig ${Number(o.validity||s.offerValidity)} Tage · ${Number(s.paymentTerm||14)} Tage Zahlungsziel.`]);if(!String(o.customer?.email||'').trim())warnings.push(['E-Mail optional','Für die PDF-Ausgabe nicht nötig, aber für den Mail-Versand erforderlich.']);return {required,warnings,ok}}
function offerCheck(o){const c=offerCheckState(o),ready=!c.required.length,entry=(type,icon,item)=>`<li class="${type}"><i>${icon}</i><span><b>${item[0]}</b><small>${item[1]}</small></span></li>`;return `<section class="offerCheck ${ready?'ready':'needsCheck'}"><div class="offerCheckHead"><div><span class="eyebrow">ANGEBOTS-CHECK</span><h3>${ready?'Bereit für PDF & Versand.':'Bitte kurz prüfen.'}</h3><p>${ready?'Die wichtigsten Angaben sind vorhanden.':'Ergänze die markierten Pflichtangaben vor der PDF-Ausgabe.'}</p></div><strong>${ready?'✓ Bereit':`${c.required.length} offen`}</strong></div><ul>${c.required.map(item=>entry('required','!',item)).join('')}${c.warnings.map(item=>entry('warning','i',item)).join('')}${ready?c.ok.map(item=>entry('ok','✓',item)).join(''):''}</ul></section>`}
function finish(o){
  const n=net(o),g=gross(o),p=profit(o),saved=db.offers.some(x=>x.id===o.id);
  return `<div class="preview">
    <div class="paper offerPreview">
      <div class="paperHead">
        <div>
          ${db.settings.logo?`<img class="offerLogo" style="display:block;max-width:120px;max-height:52px;object-fit:contain;object-position:left center;margin-bottom:12px" src="${db.settings.logo}">`:''}
    ...
    <div>
      <div class="card finishPanel">
        <span class="eyebrow">BEREIT</span>
        <h2>Sieht gut aus.</h2>
        <p class="lead">Prüfe die Vorschau kurz. Danach kannst du das Angebot ausgeben oder als offen speichern.</p>
        <div class="miniStats">
        ...
        ${offerCheck(o)}
        <div class="card" style="margin:14px 0;padding:14px;background:var(--bg)"><b>Projekt & interne Notiz</b><small style="display:block;color:var(--muted);margin:4px 0 9px">${o.reference?`Projekt / Baustelle: ${esc(o.reference)}`:'Kein Projekt hinterlegt'} · erscheint niemals im PDF</small><textarea id="offerNotes" style="width:100%;min-height:88px;border:1px solid var(--line);border-radius:9px;padding:10px;resize:vertical" placeholder="z. B. Kunde wünscht Ausführung im Oktober, Rückfrage zu Material …">${esc(o.notes||'')}</textarea><button class="ghost" style="margin-top:8px;width:100%" onclick="saveOfferNotes()">Interne Notiz speichern</button></div>
        ${attachmentsPanel(o)}
        ${saved?`<label class="field" style="margin:12px 0"><span>Status ändern</span><select onchange="setOfferStatus(this.value)">${['entwurf','offen','angenommen','abgelehnt'].map(status=>`<option value="${status}" ${o.status===status?'selected':''}>${statusLabel(status)}</option>`).join('')}</select></label><div class="followUpBox"><b>◷ Angebotsnachverfolgung</b><span>Ein Erinnerungs-Eintrag erscheint automatisch im Kalender.</span><div class="followUpQuick"><button class="textButton" onclick="quickFollowUp(3)">In 3 Tagen</button><button class="textButton" onclick="quickFollowUp(7)">In 7 Tagen</button><button class="textButton" onclick="quickFollowUp(14)">In 14 Tagen</button></div><input id="followUpDate" type="date" value="${esc(o.followUp||'')}"><button class="ghost" onclick="planFollowUp()">Nachfassen planen</button>${o.followUp?`<button class="textButton" onclick="clearFollowUp()">Nachverfolgung entfernen</button>`:''}</div>`:''}
        ${saved?`<div class="mailBox"><b>✉ Angebot per E-Mail</b><span>${o.sentAt?`Als versendet markiert: ${dateText(o.sentAt)}`:o.emailPreparedAt?`E-Mail vorbereitet: ${dateText(o.emailPreparedAt)}`:'E-Mail-Text aus deinen Einstellungen vorbereiten.'}</span><button class="ghost" onclick="prepareEmail()">✉ E-Mail vorbereiten</button>${o.sentAt?'':`<button class="textButton" onclick="markOfferSent()">Als versendet markieren</button>`}<small>PDF vorher erstellen und im Mail-Programm als Anhang ergänzen.</small></div>`:''}
...
function saveCustomer(){
  ...
  let master=db.customers.find(c=>c.id===o.customerId)||db.customers.find(c=>(c.email&&c.email.toLowerCase()===o.customer.email.toLowerCase())||(!o.customer.email&&c.name.toLowerCase()===o.customer.name.toLowerCase()));
  if(master)Object.assign(master,{name:o.customer.name,email:o.customer.email,phone:o.customer.phone,address:o.customer.address});
  else{master={id:uid(),no:db.settings.customerNext++,name:o.customer.name,company:'',contact:'',email:o.customer.email,phone:o.customer.phone,address:o.customer.address,notes:'',created:new Date().toISOString()};db.customers.push(master)}
  o.customerId=master.id;
  save();
  st.step=o.templateId?4:2;
  render()
}
function persist(){
...
function saveFollowUp(date){st.o.followUp=date;persist();db.appointments=(db.appointments||[]).filter(a=>!(a.offerId===st.o.id&&a.kind==='followup'));db.appointments.push({id:uid(),date,title:`Nachfassen: ${(db.settings.offerPrefix||'ANG-')+st.o.no}`,note:[st.o.customer.name||'Kunde',projectText(st.o),st.o.notes||''].filter(Boolean).join(' · '),color:'orange',offerId:st.o.id,kind:'followup'});save();toast('Nachverfolgung im Kalender geplant');render()}
function planFollowUp(){const date=$('#followUpDate')?.value;if(!date)return toast('Bitte ein Datum auswählen');saveFollowUp(date)}
function quickFollowUp(days){const date=new Date();date.setHours(12,0,0,0);date.setDate(date.getDate()+Number(days||0));saveFollowUp(`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`)}
...
}function offerInsights(){const active=db.offers.filter(o=>!['angenommen','abgelehnt'].includes(o.status));const today=new Date();today.setHours(0,0,0,0);const expiring=active.filter(o=>{const days=Math.ceil((offerValidUntil(o)-today)/86400000);return days>=0&&days<=7});const overdue=active.filter(o=>offerValidUntil(o)<today);const followUps=active.filter(o=>o.followUp).length;return `<div class="offerInsights"><div><small>AKTIV</small><b>${active.length}</b><span>${eur(active.reduce((sum,o)=>sum+net(o),0))} Volumen</span></div><div><small>NACHFASSEN</small><b>${followUps}</b><span>geplante Kontakte</span></div><div><small>LÄUFT BALD AB</small><b>${expiring.length}</b><span>innerhalb 7 Tagen</span></div><div class="${overdue.length?'attention':''}"><small>ABGELAUFEN</small><b>${overdue.length}</b><span>bitte prüfen</span></div></div>`}
function offers(){
...
function filterOffers(){
  ...
  const a=db.offers.filter(o=>{
    const text=`${o.no} ${o.reference||''} ${o.customer.name} ${o.customer.email} ${o.title} ${o.creatorName||''} ${o.creatorEmail||''}`.toLowerCase();
...
function customerName(customer){return customer.company||customer.name||'Unbenannter Kunde'}
function customerOffers(customer){return db.offers.filter(offer=>offer.customerId===customer.id)}
function customers(){const query=st.customerSearch.trim().toLowerCase();const list=db.customers.slice().filter(customer=>!query||[customer.no,customer.name,customer.company,customer.contact,customer.email,customer.phone,customer.address].join(' ').toLowerCase().includes(query)).sort((a,b)=>customerName(a).localeCompare(customerName(b),'de'));return `<div class="top"><div><span class="eyebrow">KUNDEN</span><h1>Deine Kunden.</h1><p class="lead">Kontakte, Angebote und Notizen an einem Ort.</p></div><button class="primary" onclick="newCustomer()">＋ Neuer Kunde</button></div><div class="customerTools"><input id="customerSearch" value="${esc(st.customerSearch)}" oninput="filterCustomers(this.value)" placeholder="⌕ Name, Firma, Ort oder Kundennummer suchen"><span>${list.length} von ${db.customers.length} Kunden</span></div><div class="card customerRows">${list.length?list.map(customer=>{const offers=customerOffers(customer),active=offers.filter(offer=>!['angenommen','abgelehnt'].includes(offer.status));return `<button class="customerRow" onclick="openCustomer('${customer.id}')"><span class="customerNo">#${customer.no}</span><div><b>${esc(customerName(customer))}</b>${customer.company&&customer.name?`<small>${esc(customer.name)}${customer.contact?` · ${esc(customer.contact)}`:''}</small>`:`<small>${esc(customer.email||customer.phone||customer.address||'Noch keine Kontaktdaten')}</small>`}</div><div class="customerContact"><span>${esc(customer.email||'Keine E-Mail')}</span><span>${esc(customer.phone||'Keine Telefonnummer')}</span></div><div><b>${offers.length}</b><small>Angebote${active.length?` · ${active.length} aktiv`:''}</small></div><strong>${eur(offers.reduce((sum,offer)=>sum+net(offer),0))}</strong><b>›</b></button>`}).join(''):`<div class="empty"><h3>Noch keine Kunden</h3><p>Lege deinen ersten Kunden an oder erstelle ein Angebot.</p><button class="primary" onclick="newCustomer()">Ersten Kunden anlegen</button></div>`}</div>`}
function filterCustomers(value){st.customerSearch=value;render();setTimeout(()=>{const input=$('#customerSearch');if(input){input.focus();input.setSelectionRange(value.length,value.length)}},0)}
function newCustomer(){st.customerEditId='new';st.page='customerDetail';render()}
function openCustomer(id){st.customerEditId=id;st.page='customerDetail';render()}
function customerDetail(){const isNew=st.customerEditId==='new';const customer=isNew?{id:'',no:db.settings.customerNext,name:'',company:'',contact:'',email:'',phone:'',address:'',notes:''}:db.customers.find(item=>item.id===st.customerEditId);if(!customer){st.page='customers';return customers()}const offers=customerOffers(customer),active=offers.filter(offer=>!['angenommen','abgelehnt'].includes(offer.status)),action=isNew?'':`<button class="primary" onclick="newOffer('${customer.id}')">＋ Neues Angebot</button>`,history=isNew?'':`<section class="section"><div class="sectionHead"><div><h2>Angebotshistorie</h2><p class="lead smallLead">Bereits erstellte Angebote dieses Kunden.</p></div></div><div class="card rows">${offers.length?offers.slice().reverse().map(row).join(''):'<div class="empty compactEmpty"><h3>Noch kein Angebot</h3><p>Starte direkt mit einem neuen Angebot für diesen Kunden.</p></div>'}</div></section>`,subtitle=isNew?'Kundendaten können auch ohne Angebot gespeichert werden.':`Kundennummer #${customer.no} · ${offers.length} Angebot${offers.length===1?'':'e'}`;return `<div class="top"><div><span class="eyebrow">KUNDENKARTEI</span><h1>${isNew?'Neuen Kunden anlegen.':esc(customerName(customer))}</h1><p class="lead">${subtitle}</p></div><div class="toolbar"><button class="ghost" onclick="go('customers')">← Kunden</button>${action}</div></div><div class="customerProfile"><section class="card formgrid"><div class="wide"><h2>Stammdaten</h2><p class="lead smallLead">Änderungen gelten für zukünftige Angebote. Bereits erstellte Angebote bleiben unverändert.</p></div>${field('Kundenname *','customerName',customer.name,'Max Mustermann')}${field('Firma','customerCompany',customer.company,'Musterbau GmbH')}${field('Ansprechpartner','customerContact',customer.contact,'Max Mustermann')}${field('E-Mail','customerEmail',customer.email,'kunde@beispiel.de')}${field('Telefon','customerPhone',customer.phone,'01234 56789')}${field('Adresse','customerAddress',customer.address,'Straße, PLZ Ort')}${textareaField('Interne Notiz','customerNotes',customer.notes)}</section><aside class="card customerProfileStats"><small>ÜBERSICHT</small><strong>${eur(offers.reduce((sum,offer)=>sum+net(offer),0))}</strong><span>Angebotsvolumen netto</span><div><b>${offers.length}</b><small>Angebote gesamt</small></div><div><b>${active.length}</b><small>aktive Angebote</small></div><button class="primary" onclick="saveCustomerProfile()">Kundendaten speichern</button></aside></div>${history}`}
function saveCustomerProfile(){const isNew=st.customerEditId==='new';const name=$('#customerName').value.trim();if(!name)return toast('Kundenname fehlt');let customer=isNew?{id:uid(),no:db.settings.customerNext++,created:new Date().toISOString()}:db.customers.find(item=>item.id===st.customerEditId);if(!customer)return;Object.assign(customer,{name,company:$('#customerCompany').value.trim(),contact:$('#customerContact').value.trim(),email:$('#customerEmail').value.trim(),phone:$('#customerPhone').value.trim(),address:$('#customerAddress').value.trim(),notes:$('#customerNotes').value.trim()});if(isNew)db.customers.push(customer);st.customerEditId=customer.id;save();toast('Kundendaten gespeichert');render()}
...
function catalogHeaderKey(value){return String(value||'').toLowerCase().replace(/[^a-z0-9äöüß]/g,'')}
function suggestCatalogMapping(headers){const find=(...names)=>headers.findIndex(header=>names.includes(catalogHeaderKey(header)));return {article:find('artikel','artikelnr','artikelnummer','articleno','sku','nummer'),name:find('bezeichnung','name','artikelbezeichnung','description','leistung'),category:find('kategorie','category','gruppe','warengruppe'),unit:find('einheit','unit','mengeneinheit'),price:find('vknetto','vk','verkaufspreis','verkaufspreisnetto','preis','price'),cost:find('ek','einkaufspreis','einkaufspreisnetto','cost','kostenpreis')}}
function catalogImportItem(row,mapping){const value=key=>mapping[key]>=0?row[mapping[key]]:'';return {id:uid(),article:String(value('article')||'').trim(),name:String(value('name')||'').trim(),category:String(value('category')||'Allgemein').trim()||'Allgemein',unit:String(value('unit')||'Stk.').trim()||'Stk.',price:catalogNumber(value('price')),cost:catalogNumber(value('cost'))}}
function catalogImportPage(){const data=st.catalogImport;if(!data){st.page='catalog';return catalogPage()}const items=data.rows.map(row=>catalogImportItem(row,data.mapping)).filter(item=>item.name);const preview=items.slice(0,6);const mapField=(key,label,required=false)=>`<label class="field"><span>${label}${required?' *':''}</span><select onchange="changeCatalogImportMap('${key}',this.value)"><option value="-1">Nicht übernehmen</option>${data.headers.map((header,index)=>`<option value="${index}" ${data.mapping[key]===index?'selected':''}>${esc(header||`Spalte ${index+1}`)}</option>`).join('')}</select></label>`;return `<div class="top"><div><span class="eyebrow">PREISKATALOG</span><h1>Import prüfen.</h1><p class="lead">${esc(data.fileName)} · ${data.rows.length} Zeilen erkannt. Erst nach deiner Bestätigung werden Artikel übernommen.</p></div><button class="ghost" onclick="cancelCatalogImport()">← Zurück</button></div><div class="importLayout"><section class="card importMap"><h2>Spalten zuordnen</h2><p class="lead smallLead">EasyOffer hat die Spalten vorausgewählt. Prüfe besonders Bezeichnung und Preise.</p><div class="formgrid">${mapField('article','Artikelnummer')}${mapField('name','Bezeichnung',true)}${mapField('category','Kategorie')}${mapField('unit','Einheit')}${mapField('price','VK netto')}${mapField('cost','EK')}</div></section><aside class="card importInfo"><small>IMPORT-VORSCHAU</small><strong>${items.length}</strong><span>gültige Positionen</span><p>Gleiche Artikelnummern werden aktualisiert. Neue Artikel werden ergänzt.</p><button class="primary" onclick="confirmCatalogImport()">${items.length} Positionen importieren</button><button class="ghost" onclick="cancelCatalogImport()">Abbrechen</button></aside></div><section class="card importPreview"><div class="sectionHead"><div><h2>Vorschau</h2><p class="lead smallLead">Die ersten ${preview.length} von ${items.length} gültigen Positionen.</p></div></div>${preview.length?`<div class="importTable"><div><span>Artikel</span><span>Bezeichnung</span><span>Kategorie</span><span>Einheit</span><span>VK netto</span><span>EK</span></div>${preview.map(item=>`<div><span>${esc(item.article||'–')}</span><b>${esc(item.name)}</b><span>${esc(item.category)}</span><span>${esc(item.unit)}</span><strong>${eur(item.price)}</strong><span>${eur(item.cost)}</span></div>`).join('')}</div>`:`<div class="empty compactEmpty"><h3>Keine gültigen Positionen erkannt</h3><p>Ordne mindestens die Spalte „Bezeichnung“ zu.</p></div>`}</section>`}
function changeCatalogImportMap(key,value){if(!st.catalogImport)return;st.catalogImport.mapping[key]=Number(value);render()}
function cancelCatalogImport(){st.catalogImport=null;st.page='catalog';render()}
async function readCatalogImport(file){try{if(file.size>10*1024*1024)throw Error('too large');const ext=(file.name.split('.').pop()||'').toLowerCase();let matrix=[];if(['xlsx','xls'].includes(ext)){if(!window.XLSX)throw Error('xlsx unavailable');const workbook=window.XLSX.read(await file.arrayBuffer(),{type:'array'});const sheet=workbook.Sheets[workbook.SheetNames[0]];matrix=window.XLSX.utils.sheet_to_json(sheet,{header:1,defval:'',raw:false})}else if(ext==='json'){const raw=JSON.parse(await file.text());const entries=Array.isArray(raw)?raw:(raw.catalog||[]);const headers=[...new Set(entries.flatMap(item=>Object.keys(item||{})))];matrix=[headers,...entries.map(item=>headers.map(header=>item?.[header]??''))]}else{const lines=(await file.text()).replace(/^\uFEFF/,'').split(/\r?\n/).filter(line=>line.trim());if(lines.length<2)throw Error('empty');const delimiter=lines[0].includes(';')?';':lines[0].includes('\t')?'\t':',';matrix=lines.map(line=>splitCatalogLine(line,delimiter))}if(matrix.length<2)throw Error('empty');const headers=matrix[0].map((header,index)=>String(header||`Spalte ${index+1}`).trim());const rows=matrix.slice(1).filter(row=>row.some(cell=>String(cell??'').trim()));st.catalogImport={fileName:file.name,headers,rows,mapping:suggestCatalogMapping(headers)};st.page='catalogImport';render()}catch(error){toast('Die Datei konnte nicht gelesen werden. Bitte CSV, Excel oder JSON prüfen.')}
}
...
function settingsContent(){
  ...
  if(st.settingsTab==='offers')return `<div class="card formgrid">
    ...
    ${textareaField('Einleitung im PDF','offerIntro',s.offerIntro||'Vielen Dank für Ihr Interesse. Gern unterbreiten wir Ihnen folgendes Angebot.')}
    ${textareaField('Footer','offerFooter',s.offerFooter)}
    ${textareaField('Bedingungen','offerTerms',s.offerTerms)}
    <div class="wide" style="border-top:1px solid var(--line);padding-top:18px"><b>Eigene Angebotsvorlagen</b><p class="lead smallLead">Speichere ein fertiges Angebot als Vorlage. Diese erscheint beim nächsten neuen Angebot.</p>${customTemplates().length?`<div style="display:grid;gap:8px">${customTemplates().map(template=>`<div style="display:flex;justify-content:space-between;gap:10px;align-items:center;padding:10px;border:1px solid var(--line);border-radius:10px"><span><b>${esc(template.name)}</b><small style="display:block;color:var(--muted)">${template.items?.length||0} gespeicherte Positionen</small></span><button class="ghost" onclick="removeOfferTemplate('${template.id}')">Löschen</button></div>`).join('')}</div>`:'<small style="color:var(--muted)">Noch keine eigenen Vorlagen gespeichert.</small>'}</div>
  </div>`;
  ...
  if(st.settingsTab==='ai')return `<div class="card formgrid">
    ...
    <div class="aiBox wide"><b>✦ Sicherer KI-Modus</b><p>EasyOffer sendet bei echter KI nur Anfrage und Artikelbezeichnungen an das geschützte Backend. Ein Baustellenfoto wird ausschließlich nach deinem Klick auf „KI prüfen“ zur Analyse übertragen. VK, EK und API-Schlüssel bleiben außerhalb der KI-Anfrage. Preise stammen weiterhin ausschließlich aus deinem Preiskatalog.</p></div>
  </div>`;
...
async function saveSettings(){
  ...
  s.company=s.company.trim()||'Dein Betrieb';
  s.next=s.offerNext;
  const renamed=await renameWorkspace(s.company);
  save();
  applyAppearance();
  toast(renamed?'Einstellungen gespeichert':'Einstellungen gespeichert – der Arbeitsbereich kann nur vom Inhaber umbenannt werden');
  render()
}
function exportData(){
...
function printOffer(){
  const o=st.o,s=db.settings,check=offerCheckState(o);if(check.required.length){toast('Bitte zuerst die Pflichtpunkte im Angebots-Check ergänzen.');return}const n=net(o),g=gross(o),offerNo=`${s.offerPrefix||'ANG-'}${o.no}`,contactName=String(o.creatorName||'').trim(),contactEmail=String(o.creatorEmail||'').trim();
...
<style>
@page{size:A4;margin:15mm}*{box-sizing:border-box}body{font:13px/1.55 Arial,Helvetica,sans-serif;color:#172033;margin:0;background:#fff}.page{max-width:800px;margin:0 auto}.header{display:flex;justify-content:space-between;gap:35px;border-bottom:3px solid #172033;padding-bottom:24px}.logo{display:block;max-width:120px;max-height:48px;object-fit:contain;object-position:left center;margin-bottom:12px}.company{font-weight:800;font-size:18px}.muted{color:#637083}.offerBox{text-align:right;min-width:185px}.offerBox strong{display:block;font-size:26px;letter-spacing:.06em}.offerNo{font-size:16px;font-weight:800;margin:4px 0 12px}.addressRow{display:flex;justify-content:space-between;gap:30px;margin:32px 0 25px}.recipient{min-width:255px}.label{display:block;text-transform:uppercase;letter-spacing:.08em;font-size:10px;font-weight:800;color:#637083;margin-bottom:6px}.title{font-size:25px;line-height:1.2;margin:22px 0 10px}.intro{margin:0 0 25px;color:#334155}.request{background:#f5f7fa;border-left:3px solid #172033;padding:11px 14px;margin:20px 0 24px;color:#334155}table{width:100%;border-collapse:collapse;margin-top:18px}th{padding:10px 8px;text-align:left;background:#172033;color:#fff;font-size:11px;letter-spacing:.04em}td{padding:12px 8px;border-bottom:1px solid #dde3eb;vertical-align:top}.right{text-align:right}.summary{width:320px;margin:26px 0 0 auto}.summary div{display:flex;justify-content:space-between;padding:7px 0}.summary .grand{border-top:2px solid #172033;margin-top:4px;padding-top:12px;font-size:18px;font-weight:800}.conditions{margin-top:38px;border-top:1px solid #dde3eb;padding-top:18px}.conditions p{margin:7px 0}.footer{display:flex;justify-content:space-between;gap:24px;margin-top:38px;padding-top:14px;border-top:1px solid #dde3eb;font-size:10px;color:#637083}.footer div:last-child{text-align:right}@media print{body{font-size:12px}.page{max-width:none}.header{break-inside:avoid}tr{break-inside:avoid}}
</style><style>
.page{max-width:790px}.header{border-bottom:0;background:#0d1728;color:#fff;padding:24px 26px;border-radius:10px 10px 0 0;align-items:flex-start}.header .muted{color:#d8e0eb}.header .logo{background:#fff;border-radius:7px;padding:5px;max-width:116px;max-height:50px;margin-bottom:10px}.offerBox{background:#ffffff14;border:1px solid #ffffff2c;border-radius:9px;padding:13px 16px}.offerBox strong{font-size:20px}.addressRow{margin:25px 0 22px}.addressRow>div{padding:15px 17px;border:1px solid #dde3eb;border-radius:9px;background:#f8fafc;min-width:0}.offerDetails{min-width:220px!important}.title{font-size:27px;margin:25px 0 8px}.intro{font-size:14px;max-width:620px}.summary{background:#f3f7f5;padding:12px 16px;border-radius:9px;border:1px solid #d9ebe1}.summary .grand{color:#0a714b}.conditions{margin-top:30px}.footer{margin-top:30px}.label{color:#0a714b}th{background:#152238}td{padding:11px 8px}
</style><style>
/* Finales A4-Layout: ruhige Hierarchie, klare Abstände, druckfreundlich */
@page{size:A4;margin:14mm}
body{font:13px/1.5 Arial,Helvetica,sans-serif;color:#1a2a3b;background:#fff}
.page{max-width:790px;margin:0 auto}
.header{position:relative;display:flex;justify-content:space-between;gap:28px;align-items:flex-start;padding:7px 0 22px;border:0;border-bottom:1px solid #dce4eb;border-radius:0;background:#fff;color:#1a2a3b}
.header:before{position:absolute;top:-7px;left:0;width:58px;height:5px;border-radius:99px;background:#119b64;content:''}
.header .company{margin-top:7px;font-size:20px;letter-spacing:-.25px}.header .muted{color:#627287;font-size:13px;line-height:1.55}.header .logo{max-width:125px;max-height:50px;margin:0 0 12px;padding:0;background:transparent;border-radius:0}
.offerBox{min-width:178px;padding:14px 16px;border:1px solid #cfe6db;border-radius:10px;background:#f3fbf7;text-align:left}.offerBox strong{color:#0a744c;font-size:12px;letter-spacing:.13em}.offerBox .offerNo{margin:5px 0 11px;color:#17283b;font-size:17px}.offerBox .muted{font-size:10px;text-transform:uppercase;letter-spacing:.08em}
.addressRow{display:grid;grid-template-columns:1.2fr .8fr;gap:14px;margin:27px 0 25px}.addressRow>div{min-width:0;padding:15px 17px;border:1px solid #dfe6ed;border-radius:9px;background:#fbfcfd}.offerDetails{background:#f7fafc!important;color:#506177;line-height:1.65}.label{margin-bottom:7px;color:#0a744c;font-size:10px;font-weight:800;letter-spacing:.1em}.contactLabel{display:block;margin-top:3px}.title{margin:25px 0 8px;color:#10243a;font-size:29px;letter-spacing:-.55px;line-height:1.17}.intro{max-width:640px;margin:0 0 25px;color:#4c5d70;font-size:14px;line-height:1.6}
table{margin-top:18px;border:1px solid #dfe6ed;border-radius:9px;overflow:hidden}thead{display:table-header-group}th{padding:11px 10px;background:#15283e;color:#fff;font-size:10px;letter-spacing:.07em}td{padding:12px 10px;border-bottom:1px solid #e4e9ef;color:#26384c}tbody tr:nth-child(even){background:#f8fafc}tbody tr:last-child td{border-bottom:0}tr{break-inside:avoid}.right{text-align:right}
.summary{width:325px;margin:28px 0 0 auto;padding:12px 16px;border:1px solid #d4e9df;border-radius:10px;background:#f4fbf7}.summary div{padding:6px 0}.summary .grand{margin-top:5px;padding-top:12px;border-top:1px solid #acd7c0;color:#0a714b;font-size:19px}.conditions{margin-top:33px;padding-top:17px;border-top:1px solid #dfe6ed;color:#506177}.conditions p{margin:7px 0}.footer{margin-top:32px;padding-top:13px;border-top:1px solid #dfe6ed;color:#718095;font-size:10px}.footer div:last-child{text-align:right}
@media print{body{font-size:13px}.page{max-width:none}.header,.addressRow,.summary{break-inside:avoid}.header{padding-top:7px}}
...
<section class="addressRow"><div class="recipient"><span class="label">Angebot für</span><b>${esc(o.customer.name)}</b><br>${esc(o.customer.address).replace(/\n/g,'<br>')}<br>${esc(o.customer.email)}${o.customer.phone?`<br>${esc(o.customer.phone)}`:''}</div><div class="offerDetails"><span class="label">Angebotsdetails</span>${o.reference?`Ihre Referenz: ${esc(o.reference)}<br>`:''}Gültig bis: ${validUntil.toLocaleDateString(s.language==='en'?'en-GB':'de-DE',dateFormat)}<br>Zahlungsziel: ${Number(s.paymentTerm||14)} Tage${contactName||contactEmail?`<br><br><span class="label contactLabel">Ihr Ansprechpartner</span><b>${esc(contactName||contactEmail)}</b>${contactEmail?`<br>${esc(contactEmail)}`:''}`:''}</div></section>
<h1 class="title">${esc(o.title||s.offerTitle||'Angebot')}</h1><p class="intro">${esc(s.offerIntro||'Vielen Dank für Ihr Interesse. Gern unterbreiten wir Ihnen folgendes Angebot.').replace(/\n/g,'<br>')}</p>
...
function applyAppearance(){
  ...
  if(!style){style=document.createElement('style');style.id='easyofferAppearanceStyles';style.textContent=`
    ...
    :root[data-theme="dark"] .analysisCard{background:#172033;color:#f3f6fb;border-color:#2b394d}:root[data-theme="dark"] .analysisNotice.ok{background:#10261e;border-color:#24533e}:root[data-theme="dark"] .analysisNotice.warning{background:#302615;border-color:#765522}:root[data-theme="dark"] .analysisList,:root[data-theme="dark"] .analysisListHead,:root[data-theme="dark"] .analysisItem{border-color:#34445b}:root[data-theme="dark"] .analysisQty{background:#123a2b;color:#8ce6b6}:root[data-theme="dark"] .suggestionTag{background:#123a2b;color:#8ce6b6}:root[data-theme="dark"] .analysisCheck{background:#111b2a}:root[data-theme="dark"] .missingChips span{background:#4a3511;color:#ffd788}
    ...
    .offerAttachments{margin:14px 0;padding:14px;border:1px solid var(--line);border-radius:12px;background:var(--bg);display:grid;gap:8px}.offerAttachments>div{display:flex;justify-content:space-between;gap:10px;align-items:center}.offerAttachments>div small{display:block;color:var(--muted);font-size:12px;margin-top:3px}.attachmentUpload{display:inline-flex;width:max-content;cursor:pointer}.attachmentUpload input{display:none}.attachmentUpload span{border:1px solid var(--line);background:var(--card);border-radius:9px;padding:8px 10px;font-size:13px;font-weight:800}.attachmentHint,.attachmentEmpty{font-size:12px!important;color:var(--muted)!important;margin:0}.attachmentList{display:grid;gap:6px;margin-top:3px}.attachmentRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap;border-top:1px solid var(--line);padding-top:8px}.attachmentRow i{font-size:11px;font-style:normal;font-weight:900;min-width:28px;height:28px;display:grid;place-items:center;border-radius:7px;background:#eaf6ef;color:var(--accent2)}.attachmentRow>span{display:grid;min-width:0;flex:1}.attachmentRow>span b{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.attachmentRow>span small{font-size:11px;color:var(--muted)}.attachmentRow .textButton{font-size:12px;white-space:nowrap}.dangerText{color:#b42318!important}.imageAnalysisResult{width:100%;display:grid;gap:4px;padding:9px 10px;margin-top:2px;border-radius:9px;background:#eef8f2;color:#14532d;font-size:12px}.imageAnalysisResult small{color:#3f6750!important}.imageAnalysisResult .textButton{width:max-content;color:var(--accent2);font-weight:800;padding:2px 0}
    .offerAttachments{margin:14px 0;padding:14px;border:1px solid var(--line);border-radius:12px;background:var(--bg);display:grid;gap:8px}.offerAttachments>div{display:flex;justify-content:space-between;gap:10px;align-items:center}.offerAttachments>div small{display:block;color:var(--muted);font-size:12px;margin-top:3px}.attachmentUpload{display:inline-flex;width:max-content;cursor:pointer}.attachmentUpload input{display:none}.attachmentUpload span{border:1px solid var(--line);background:var(--card);border-radius:9px;padding:8px 10px;font-size:13px;font-weight:800}.attachmentHint,.attachmentEmpty{font-size:12px!important;color:var(--muted)!important;margin:0}.attachmentList{display:grid;gap:6px;margin-top:3px}.attachmentRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap;border-top:1px solid var(--line);padding-top:8px}.attachmentRow i{font-size:11px;font-style:normal;font-weight:900;min-width:28px;height:28px;display:grid;place-items:center;border-radius:7px;background:#eaf6ef;color:var(--accent2)}.attachmentRow>span{display:grid;min-width:0;flex:1}.attachmentRow>span b{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.attachmentRow>span small{font-size:11px;color:var(--muted)}.attachmentRow .textButton{font-size:12px;white-space:nowrap}.dangerText{color:#b42318!important}.imageAnalysisResult{width:100%;display:grid;gap:4px;padding:9px 10px;margin-top:2px;border-radius:9px;background:#eef8f2;color:#14532d;font-size:12px}.imageAnalysisResult small{color:#3f6750!important}.imageAnalysisResult .textButton{width:max-content;color:var(--accent2);font-weight:800;padding:2px 0}
    .offerPreview{min-height:0!important;max-height:620px;overflow:auto;padding:30px 34px!important;border-radius:16px}.offerPreview::-webkit-scrollbar{width:8px}.offerPreview::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:999px}.finishPanel{padding:26px!important}.finishPanel>h2{margin:4px 0 0;font-size:29px}.finishPanel>.lead{margin:0 0 3px;line-height:1.45}.miniStats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:6px 0 2px}.miniStats>div{padding:10px;border:1px solid var(--line);border-radius:10px;background:var(--bg)}.miniStats small{display:block;color:var(--muted);font-size:10px;font-weight:900;letter-spacing:.06em}.miniStats b{display:block;margin-top:4px;font-size:15px;white-space:nowrap}.finishPanel>.primary{margin-top:2px}.finishPanel>.ghost,.finishPanel>.textButton{min-height:44px}.offerCreator{color:var(--accent2)!important;font-weight:700}.teamProfile{display:grid;gap:7px;margin:18px 0;padding:15px;border:1px solid var(--line);border-radius:12px;background:var(--bg)}.teamProfile>small{color:var(--muted);font-size:12px}.teamProfile>div{display:flex;gap:8px;align-items:center}.teamProfile input{min-width:0;flex:1;border:1px solid var(--line);border-radius:9px;background:var(--card);color:var(--ink);padding:10px 11px}.teamProfile .ghost{white-space:nowrap}.offerCheck{margin:14px 0;padding:14px;border:1px solid var(--line);border-radius:12px;background:var(--bg)}.offerCheck.ready{border-color:#c5ead6;background:#f1fbf5}.offerCheck.needsCheck{border-color:#f0d39a;background:#fffaf0}.offerCheckHead{display:flex;justify-content:space-between;gap:10px}.offerCheck h3{margin:1px 0 4px;font-size:17px}.offerCheck p{margin:0;color:var(--muted);font-size:12px;line-height:1.4}.offerCheckHead>strong{height:max-content;padding:6px 8px;border-radius:8px;background:#dff7e9;color:#08764d;font-size:11px;white-space:nowrap}.needsCheck .offerCheckHead>strong{background:#ffedc9;color:#926000}.offerCheck ul{display:grid;gap:6px;margin:12px 0 0;padding:0;list-style:none}.offerCheck li{display:flex;gap:8px;align-items:flex-start;padding-top:7px;border-top:1px solid #dce8e1}.offerCheck li i{display:grid;place-items:center;flex:0 0 18px;width:18px;height:18px;border-radius:50%;background:#d9f6e5;color:#08764d;font-style:normal;font-size:11px;font-weight:900}.offerCheck li.required i{background:#ffe1a5;color:#8a5d00}.offerCheck li.warning i{background:#fff0cc;color:#986500}.offerCheck li b,.offerCheck li small{display:block}.offerCheck li b{font-size:12px}.offerCheck li small{margin-top:1px;color:var(--muted);font-size:11px;line-height:1.35}.setupTips{margin-top:28px;padding:22px 24px!important;background:linear-gradient(135deg,#f1fbf6,#fff)!important;border-color:#ccebd9!important}.setupTipsHead{display:flex;justify-content:space-between;gap:15px}.setupTips h2{margin:0 0 5px;font-size:21px}.setupTips p{margin:0;color:var(--muted);font-size:14px}.tipClose{width:30px;height:30px;border:0;border-radius:8px;background:transparent;color:var(--muted);font-size:24px;line-height:22px;cursor:pointer}.tipClose:hover{background:#e3f4ea;color:var(--ink)}.setupTips ol{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:20px 0 0;padding:0;list-style:none}.setupTips li{min-height:76px;display:grid;grid-template-columns:25px 1fr auto;align-items:start;gap:8px;padding:12px;border:1px solid var(--line);border-radius:11px;background:var(--card)}.setupTips li i{display:grid;place-items:center;width:21px;height:21px;border-radius:50%;background:#e5eaf0;color:#6b7787;font-style:normal;font-size:13px;font-weight:900}.setupTips li.done i{background:#d9f8e7;color:#078655}.setupTips li b,.setupTips li small{display:block}.setupTips li b{font-size:13px}.setupTips li small{margin-top:3px;color:var(--muted);font-size:11px;line-height:1.35}.setupTips .textButton{align-self:end;white-space:nowrap;font-size:12px;color:var(--accent2);font-weight:800}@media(max-width:850px){.offerPreview{max-height:none;overflow:visible;padding:23px!important}.miniStats{grid-template-columns:1fr 1fr 1fr}.finishPanel{padding:18px!important}.setupTips ol{grid-template-columns:1fr}.setupTips li{min-height:auto}.teamProfile>div{align-items:stretch;flex-direction:column}.teamProfile .ghost{width:100%}}
...
function render(){
  ...
  else if(st.page==='customerDetail')c=customerDetail();
  else if(st.page==='catalog')c=catalogPage();
  else if(st.page==='catalogImport')c=catalogImportPage();
...
Object.assign(window,{
  go,newOffer,openSetupTip,dismissSetupTips,saveCustomer,selectExistingCustomer,applyOfferTemplate,saveOfferAsTemplate,removeOfferTemplate,analyze,refineAnalysis,uploadAttachments,openAttachment,removeAttachment,analyzeAttachment,applyImageAnalysis,back,newCustomer,openCustomer,filterCustomers,saveCustomerProfile,
  ...
  printOffer,markOpen,persist,deleteCurrentOffer,authScreen,submitAuth,signOut,resetScreen,sendReset,passwordUpdateScreen,updatePassword,createInvite,copyInvite,saveMyOfferContact,onboardingBack,onboardingNext
});
bootApp();
