...
async function renameWorkspace(name){if(!cloud.client||!cloud.workspace||cloud.workspace.role!=='owner'||!name||cloud.workspace.name===name)return true;const {error}=await cloud.client.from('workspaces').update({name}).eq('id',cloud.workspace.id);if(error){console.warn('Arbeitsbereich konnte nicht umbenannt werden',error.message);return false}cloud.workspace.name=name;return true}
