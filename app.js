...
let cloud={client:null,user:null,ready:false,syncTimer:0,status:'saved'};
...
async function syncCloud(){if(!cloud.ready||!cloud.client||!cloud.user)return;const {error}=await cloud.client.from('easyoffer_data').upsert({user_id:cloud.user.id,data:db,updated_at:new Date().toISOString()},{onConflict:'user_id'});cloud.status=error?'error':'saved';updateCloudStatus();if(error)console.warn('Cloud-Speicherung fehlgeschlagen',error.message)}
...
Object.assign(window,{
  ...
  printOffer,markOpen,persist,authScreen,submitAuth,signOut,resetScreen,sendReset,passwordUpdateScreen,updatePassword
});
bootApp();
