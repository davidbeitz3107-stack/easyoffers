...
function setupTips(){const s=db.settings,company=!!(s.company&&s.company!=='Dein Betrieb'),catalog=db.catalog.length>0,offer=db.offers.length>0;if(s.setupTipsDismissed||(company&&catalog&&offer))return '';const count=[company,catalog,offer].filter(Boolean).length;const item=(done,title,text,action,target)=>`<li class="${done?'done':''}"><i>${done?'✓':'•'}</i><div><b>${title}</b><small>${text}</small></div>${done?'':`<button class="textButton" onclick="openSetupTip('${target}')">${action} →</button>`}</li>`;return `<section class="setupTips card"><div class="setupTipsHead"><div><span class="eyebrow">SCHNELL STARTKLAR</span><h2>Deine ersten Schritte</h2><p>${count}/3 erledigt – damit dein erster Entwurf sofort professionell aussieht.</p></div><button class="tipClose" onclick="dismissSetupTips()" title="Hinweise ausblenden">×</button></div><ol>${item(company,'Betriebsdaten prüfen','Firma, Kontakt und Adresse für dein PDF.','Einstellungen','company')}${item(catalog,'Preiskatalog anlegen','Eigene Preise bleiben immer die Grundlage.','Katalog öffnen','catalog')}${item(offer,'Erstes Angebot erstellen','Der Assistent führt dich Schritt für Schritt durch.','Angebot starten','offer')}</ol></section>`}
