...
const SYSTEM_TEMPLATES=[
  {id:'system-free',name:'Freies Angebot',title:'Angebot',description:'Starte ohne vorbereitete Positionen.',terms:[]},
  {id:'system-maintenance',name:'Wartung',title:'Angebot – Wartung',description:'Wartung, Anfahrt und typische Arbeitspositionen aus deinem Katalog.',terms:['wartung','inspektion','anfahrt']},
...
function customer(o){
  ...
  const templatePicker=`<div class="card" style="margin-bottom:16px"><div class="sectionHead"><div><b>Vorlage wählen <span style="font-weight:500;color:var(--muted)">(optional)</span></b><small style="display:block;color:var(--muted);margin-top:4px">Vorlagen übernehmen ausschließlich Preise aus deinem eigenen Katalog.</small></div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:9px;margin-top:12px">${templates.map(template=>`<button class="${o.templateId===template.id?'primary':'ghost'}" style="text-align:left;padding:11px" onclick="applyOfferTemplate('${template.id}')"><b>${esc(template.name)}</b><small style="display:block;opacity:.8;margin-top:3px">${esc(template.description||`${template.items?.length||0} Positionen gespeichert`)}</small></button>`).join('')}</div>${selected?`<small style="display:block;margin-top:10px;color:var(--accent2)">✓ ${esc(selected.name)} ist ausgewählt – nach dem Kunden geht es direkt zur Kalkulation.</small>`:''}</div>`;
...
function applyAppearance(){
  ...
  if(!style){style=document.createElement('style');style.id='easyofferAppearanceStyles';style.textContent=`
    ...
    .analysisNotice.question{background:#f2f7ff;border-color:#bdd2f2}.analysisNotice.question ol{margin:10px 0 14px;padding-left:21px;font-weight:700;line-height:1.65}.analysisNotice.question textarea{min-height:92px}.refineBtn{margin:12px 0 8px}
    :root[data-theme="dark"] .analysisNotice.question{background:#142544;border-color:#355b95}
